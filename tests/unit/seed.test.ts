...
function makeFixtureRepo(): { url: string; firstCommit: string; secondCommit: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-test-fixture-"));
...
describe("resolveResultsDirSeed: validation", () => {
  ...
  test("missing audit-state.json → fatal", async () => {
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-empty-"));
    try {
      await expect(
  ...
  test("audit-state with empty audits[] → fatal", async () => {
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-emptylist-"));
    try {
  ...
  test("null repository → fatal", async () => {
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-norepo-"));
    try {
  ...
  test("null commit → fatal", async () => {
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-nocommit-"));
    try {
  ...
  test("--from-audit referencing unknown id → fatal", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-badid-"));
    try {
  ...
  test("--target points at a non-empty directory → fatal", async () => {
    const fixture = makeFixtureRepo();
    const dir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-fulltarget-"));
    const target = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-target-"));
...
describe("resolveResultsDirSeed: clone + sync-back happy path", () => {
  test("clones at pinned commit, copies vigolium-results/ in, syncs new files back, removes temp clone", async () => {
    const fixture = makeFixtureRepo();
    const fromDir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-from-"));
    try {
      ...
      // vigolium-results/ landed in the clone.
      expect(
        readFileSync(join(handle.clonedTargetDir, "vigolium-results", "findings", "H1-test", "report.md"), "utf8"),
      ).toBe("original report\n");

      // Simulate a confirm-mode run writing into the clone's vigolium-results/.
      writeFileSync(join(handle.clonedTargetDir, "vigolium-results", "confirmation-report.md"), "verified\n");
      writeFileSync(
        join(handle.clonedTargetDir, "vigolium-results", "findings", "H1-test", "report.md"),
        "updated report\n",
      );
  ...
  test("--keep-clone leaves the clone in place", async () => {
    const fixture = makeFixtureRepo();
    const fromDir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-keep-"));
    try {
  ...
  test("--target overrides clone destination; clone is preserved", async () => {
    const fixture = makeFixtureRepo();
    const fromDir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-target-from-"));
    const targetParent = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-target-parent-"));
  ...
  test("supports nested vigolium-results/ layout (project dir + vigolium-results/ subdir)", async () => {
    const fixture = makeFixtureRepo();
    const projectDir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-nested-"));
    ...
    try {
      ...
      // vigolium-results/ in the clone matches the input's vigolium-results/ contents.
      expect(readFileSync(join(handle.clonedTargetDir, "vigolium-results", "marker.txt"), "utf8")).toBe(
        "marker\n",
      );
      // Confirm sync-back lands in the original vigolium-results/ subdir.
      writeFileSync(join(handle.clonedTargetDir, "vigolium-results", "new.txt"), "new\n");
      await handle.cleanup();
      expect(readFileSync(join(resultsDir, "new.txt"), "utf8")).toBe("new\n");
      // The project dir itself is not the vigolium-results dir, so files there are untouched.
      expect(readdirSync(projectDir).sort()).toEqual(["vigolium-results"]);
    } finally {
      fixture.cleanup();
  ...
  test("cleanup is idempotent", async () => {
    const fixture = makeFixtureRepo();
    const fromDir = mkdtempSync(join(tmpdir(), "vigolium-audit-seed-idem-"));
    try {
      writeAuditStateAt(fromDir, makeAudit({ repository: fixture.url, commit: fixture.secondCommit }));
      const handle = await resolveResultsDirSeed({
        fromResultsDir: fromDir,
      ...
      writeFileSync(join(handle.clonedTargetDir, "vigolium-results", "marker.txt"), "x\n");
      await handle.cleanup();
