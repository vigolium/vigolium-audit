...
/**
 * Print a compact summary of the latest audit in a target's `vigolium-results/` folder.
 * Read-only: never modifies state. Falls back to a friendly hint when no
 * vigolium-audit state exists in the target.
 */
export async function statusCommand(targetPath: string, opts: StatusOptions = {}): Promise<void> {
  ...
  if (!existsSync(resultsDir)) {
    return fail(opts, `no vigolium-results/ directory at ${targetDir} — run an audit first`);
  }
  ...
  console.log(chalk.bold(`\nvigolium-audit — status for ${chalk.cyan(targetDir)}`));
