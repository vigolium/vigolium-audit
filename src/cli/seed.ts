...
export interface ResultsDirSeedHandle {
  ...
  summary: string;
  /**
   * Sync the clone's vigolium-results/ back to the original input dir, then remove the
...
export async function resolveResultsDirSeed(
  ...
  // Support both layouts: the input path can be the vigolium-results/ dir itself
  // (audit-state.json directly inside) or a project dir that contains a
  // vigolium-results/ subdir. Whichever holds audit-state.json wins.
  const resultsDir = findResultsDir(fromDir);
  if (resultsDir === null) {
    throw new Error(
      `--from-results-dir: no audit-state.json found at ${join(fromDir, "audit-state.json")} ` +
        `or ${join(fromDir, "vigolium-results", "audit-state.json")}`,
    );
  }
  ...
  const destResultsDir = join(cloneRoot, "vigolium-results");
  ...
  const summary =
    ...
    `vigolium-results/ copied from ${resultsDir}`;
...
function findResultsDir(input: string): string | null {
  ...
  const nested = join(input, "vigolium-results");
...
function resolveCloneDestination(targetOverride: string | undefined, repository: string): string {
  ...
  const parent = mkdtempSync(join(tmpdir(), "vigolium-audit-clone-"));
