...
`vigolium-results/audit-state.json` is an append-only audit history. Each audit run is appended as a new
...
load the KB sections from `vigolium-results/attack-surface/knowledge-base-report.md` as the starting knowledge base.

For Phase 4, `reports_generated` must include `vigolium-results/codeql-artifacts/entry-points.json`,
`vigolium-results/codeql-artifacts/sinks.json`, `vigolium-results/codeql-artifacts/call-graph-slices.json`, and
`vigolium-results/codeql-artifacts/flow-paths-all-severities.md`, plus the `## Static Analysis Summary`
section written to `vigolium-results/attack-surface/knowledge-base-report.md`. Missing any causes `validation_passed: false`.
...
"codeql_structural": {
  ...
  "db_path": "vigolium-results/codeql-artifacts/db/"
}
```
...
Debate: <path to chamber debate transcript, e.g., vigolium-results/chamber-workspace/chamber-01/debate.md>
...
- **Capture evidence.** For Critical and High findings, save execution output to `vigolium-results/findings/<ID>-<slug>/evidence/` (screenshots, response captures, or log snippets).
...
Setup commands: see `vigolium-results/real-env-evidence/<slug>/setup.sh`
...
Evidence: vigolium-results/real-env-evidence/<slug>/
...
## Pentest-Style Final Report Template (`vigolium-results/final-audit-report.md`)
...
- **Detailed Report:** [vigolium-results/findings/[ID]-[slug]/report.md]
- **Proof of Concept:** [vigolium-results/findings/[ID]-[slug]/poc.py]
...
```bash
jq 'length' vigolium-results/codeql-artifacts/entry-points.json
jq 'length' vigolium-results/codeql-artifacts/sinks.json
jq '[.[] | select(.reachable == true)] | length' vigolium-results/codeql-artifacts/call-graph-slices.json
jq '.runs[0].results | length' vigolium-results/codeql-artifacts/flow-paths-raw.sarif
```
...
File: `vigolium-results/chamber-workspace/<chamber-id>/debate.md`
