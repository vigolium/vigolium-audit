...
mkdir -p vigolium-results/ vigolium-results/findings-draft/
...
if [ -f vigolium-results/audit-state.json ]; then
  # Append new entry to existing audits array
  python3 -c "
import json, sys
data = json.load(open('vigolium-results/audit-state.json'))
data['audits'].append(json.loads(sys.argv[1]))
json.dump(data, open('vigolium-results/audit-state.json', 'w'), indent=2)
" "$NEW_ENTRY"
else
  # Create new file with first entry
  python3 -c "
import json, sys
json.dump({'audits': [json.loads(sys.argv[1])]}, open('vigolium-results/audit-state.json', 'w'), indent=2)
" "$NEW_ENTRY"
fi
```
...
git log --oneline v<vulnerable>..v<patched> -- src/vigolium-results/ src/auth/ src/validation/

# Diff the full range
git diff v<vulnerable>..v<patched> -- <relevant-paths>
```

**Output**: `## Advisory Intelligence` section of `vigolium-results/attack-surface/knowledge-base-report.md`, populated with advisory inventory, architecture intelligence, vulnerability class patterns, and supply chain risk summary.
...
**Output**: `## Bypass Analysis` section of `vigolium-results/attack-surface/knowledge-base-report.md`
...
Write results to the `## Domain Attack Research` section of `vigolium-results/attack-surface/knowledge-base-report.md`.

**Output**: `vigolium-results/attack-surface/knowledge-base-report.md` with all Phase 3 sections populated (Project Classification, Architecture, Trust Boundaries, DFD/CFD Slices, Threat Model, Attack Surface, Domain Attack Research, Specs/RFCs, Dependencies, Phase 4 Modeling Targets)
...
- store CodeQL artifacts under `vigolium-results/codeql-queries/`
- store Semgrep artifacts under `vigolium-results/semgrep-rules/`
...
`vigolium-results/attack-surface/knowledge-base-report.md`.
...
rm -rf vigolium-results/codeql-db/ vigolium-results/codeql-db-*/
...
jq -s '{ version: "2.1.0", runs: [ .[].runs[] ] }' \
  vigolium-results/codeql-res/*.sarif \
  vigolium-results/semgrep-res/*.sarif \
  > vigolium-results/merged-results.sarif
```

**Output**: `## Static Analysis Summary` and `## GitHub Actions Audit` sections of
`vigolium-results/attack-surface/knowledge-base-report.md`. The Static Analysis Summary must record:
...
Write enrichment verdicts to the `## SAST Enrichment` section of `vigolium-results/attack-surface/knowledge-base-report.md`.
...
`vigolium-results/attack-surface/knowledge-base-report.md`. Use the Mode C attack taxonomy and manual review checklist
...
**Output**: `## Spec Gap Analysis` section of `vigolium-results/attack-surface/knowledge-base-report.md`. If no specs were identified in Phase 3, mark "None identified" and skip.
...
**Input**: `vigolium-results/attack-surface/knowledge-base-report.md` (all sections from phases 1-6),
`vigolium-results/codeql-artifacts/` (structural artifacts from Phase 4)
...
5. Create `vigolium-results/chamber-workspace/` and `vigolium-results/attack-pattern-registry.json`
...
```bash
mkdir -p vigolium-results/chamber-workspace/<chamber-id>/{evidence,variant-candidates}
```
...
`vigolium-results/attack-pattern-registry.json` stores confirmed patterns with detection signatures
...
**Output**: `vigolium-results/findings-draft/p7-<NNN>-<slug>.md` (Medium+ only),
`vigolium-results/chamber-workspace/<chamber-id>/debate.md` (audit artifacts),
`vigolium-results/attack-pattern-registry.json`,
...
`vigolium-results/findings-draft/p7-*.md` file immediately. Add:
...
**Full review output**: `vigolium-results/adversarial-reviews/<slug>-review.md` using the template
from `report-templates.md`.

**Output**: updated `vigolium-results/findings-draft/` files (CRITICAL/HIGH with cold verification
verdicts), `vigolium-results/adversarial-reviews/<slug>-review.md` per CRITICAL/HIGH VALID finding
...
1. Read the matching pattern from `vigolium-results/attack-pattern-registry.json`
...
matching `vigolium-results/attack-pattern-registry.json` pattern's `confirmed_instances`. Variants then
...
`vigolium-results/attack-pattern-registry.json` updated with confirmed variant instances
...
1. List all files in `vigolium-results/findings-draft/` with `Verdict: VALID`.
...
3. For each confirmed draft, create `vigolium-results/findings/<ID>-<slug>/` and copy the draft as the basis for the `vuln-report` output.
...
**Real-environment execution mandate for CRITICAL/HIGH findings**: For every CRITICAL or HIGH finding promoted to `vigolium-results/findings/`, PoC execution in a real environment is required before the final report is generated. Reuse the Stage 2 adversarial environment if it was successfully provisioned; otherwise provision a new environment following `real-env-validation.md`.

Capture evidence in `vigolium-results/findings/<ID>-<slug>/evidence/`:
```
vigolium-results/findings/<ID>-<slug>/evidence/
...
-   **Naming Convention**: Save each report to `vigolium-results/findings/<ID>-<slug>/report.md`.
...
This is the mandatory final step to synthesize the entire audit. Generate `vigolium-results/final-audit-report.md` using the template in `audit/references/report-templates.md`.
...
```bash
ls vigolium-results/attack-surface/knowledge-base-report.md vigolium-results/final-audit-report.md vigolium-results/findings/
```
