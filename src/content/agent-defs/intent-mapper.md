...
- **Output path**: where to write the corpus JSON (`vigolium-results/confirm-workspace/intent-corpus.json` or `vigolium-results/attack-surface/intent-corpus.json` depending on mode).
- **Findings inventory path** (optional): `vigolium-results/confirm-workspace/findings-inventory.json`. If present, you also run a **cross-check pass** (see Step 4) and write per-finding verdicts.
...
Skip generated, vendored, and lockfile directories: `node_modules/`, `vendor/`, `.git/`, `dist/`, `build/`, `target/`, `vigolium-results/` itself.
