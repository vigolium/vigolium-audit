---
description: Final report compilation agent that collects confirmed findings from vigolium-results/findings/ and theoretical/unconfirmed findings from vigolium-results/findings-theoretical/, reads adversarial consensus documents and debate transcripts, produces the consolidated pentest-style vigolium-results/final-audit-report.md with a separate Theoretical / Unconfirmed Findings section, and runs all consistency checks
---
...
- `vigolium-results/findings/` — directories for each **confirmed** finding (`C1-<slug>/`, `H1-<slug>/`, `M1-<slug>/`): a PoC was executed (`PoC-Status: executed`). Each contains:
...
- `vigolium-results/attack-surface/knowledge-base-report.md` — the knowledge base with all phase sections
- `vigolium-results/attack-surface/intent-reconciliation.md` — (balanced B6 / deep D10, optional) per-finding reconciliation of findings against documented project intent. Present unless Intent Reconciliation was skipped (skip-and-continue).
- `vigolium-results/chamber-workspace/` — debate transcripts (for methodology context, if not yet cleaned up)
- `vigolium-results/adversarial-reviews/` — cold verification results (if not yet cleaned up)
- `vigolium-results/attack-pattern-registry.json` — confirmed attack patterns (if not yet cleaned up)

## Report Generation

### 1. Collect Findings

List all directories in `vigolium-results/findings/` (the confirmed bucket). For each:
...
If `vigolium-results/attack-surface/intent-reconciliation.md` exists, read it and carry its
...
Write `vigolium-results/final-audit-report.md` using this Pentest-Style template:
...
- **PoC Status:** executed
- **Detailed Report:** vigolium-results/findings/C1-<slug>/report.md
- **Proof of Concept:** vigolium-results/findings/C1-<slug>/poc.{py|sh|js}
...
See individual variant reports: vigolium-results/findings/H2-<slug>/report.md
...
- **Detailed Report:** vigolium-results/findings-theoretical/M1-<slug>/report.md
```
...
1. **Finding ID cross-reference**: every ID in the report matches a directory in `vigolium-results/findings/` or `vigolium-results/findings-theoretical/`; IDs are unique across both buckets (no `C1` in both)
...
Include a brief methodology appendix noting (read from `vigolium-results/chamber-workspace/` if it exists, or from individual `debate.md` files in finding directories):
...
- `vigolium-results/final-audit-report.md` — the consolidated pentest-style report
