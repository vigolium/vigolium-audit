---
description: Cross-agent reinvest mode. Re-verifies CRITICAL and HIGH findings from a prior audit using a DIFFERENT agent platform / model than the one that originally produced them, surfacing model-specific blind spots. Reads the existing vigolium-results/findings/ directory; does NOT re-run discovery, KB construction, probe, or chamber phases.
...
- Existing findings: !`ls vigolium-results/findings/ 2>/dev/null | grep -E "^(C|H)[0-9]" | head -20 || echo "No CRIT/HIGH findings"`
- Prior reinvest waves: !`ls vigolium-results/findings/*/wave-*-verdict.md 2>/dev/null | wc -l | xargs -I{} echo "{} prior wave verdicts on disk"`

## Your Task

Re-verify the CRITICAL and HIGH findings in `vigolium-results/findings/` using a different agent platform than the one that produced them. The goal is to surface model-specific blind spots — Opus and Codex agree on most findings but diverge on a meaningful minority, and that disagreement is exactly what this mode exists to capture.

This mode does NOT run discovery, KB construction, static analysis, probe teams, or review chambers. It only spawns `vigolium-audit:cross-verifier` per finding directory and surfaces the resulting consensus (or disagreement) in the final report.

### When to Use Reinvest

Run `/vigolium-audit:reinvest` after a deep or balanced audit completes, then re-launch `vigolium-audit` from the SAME vigolium-results/ directory but with a different `--agent` flag than the original audit. Concrete flow:

```
# Original audit (produced vigolium-results/findings/ with C1, H1, H2, ...)
vigolium-audit run --mode deep --agent claude

# Cross-agent reinvest using Codex on top of those findings
vigolium-audit run --mode reinvest --agent codex
```
...
- **Empty**: reinvest every CRITICAL and HIGH finding under `vigolium-results/findings/`. MEDIUM findings are intentionally excluded — they are too numerous and the per-finding cost of cross-verifier is too high to justify a mass second pass at MEDIUM.

### Pre-Flight Check

If `vigolium-results/findings/` does not exist or contains no `C*-*` / `H*-*` directories, exit with a message telling the user to run `/vigolium-audit:deep` or `/vigolium-audit:balanced` first.

If `vigolium-results/audit-state.json` does not exist, exit — this mode requires a prior audit to compute `parent_audit_id`.

Read `vigolium-results/audit-state.json`. The most recent audit's `agent_sdk` and `model` are the BASELINE this reinvest is comparing against. If the current `$AGENT_SDK` env var (set by the CLI) matches the baseline `agent_sdk`, prompt the user with `AskUserQuestion`:

> "The most recent audit ran on `<baseline-agent>` and you are reinvesting on the same platform. The cross-agent value of /vigolium-audit:reinvest comes from swapping platforms (Claude ↔ Codex). What would you like to do?"
> Options:
...
For each in-scope finding directory `vigolium-results/findings/<ID>-<slug>/`:
...
Append a new entry to `audits[]` in `vigolium-results/audit-state.json` BEFORE dispatching wave-verifiers:
...
```bash
ls vigolium-results/findings/ | grep -E "^(C|H)[0-9]+-" | sort
```
...
Write the summary to `vigolium-results/reinvest-report.md`:
...
The original `vigolium-results/final-audit-report.md` is NOT modified by this mode. Reinvest produces its own delta report (`vigolium-results/reinvest-report.md`) so the original report remains the authoritative artefact for that audit's commit.
