...
- Existing audit state: !`cat vigolium-results/audit-state.json 2>/dev/null | head -40 || echo "No prior audit state"`
- Prior longshot run: !`test -f vigolium-results/longshot/targets.json && echo "present ($(jq -r '.targets | length' vigolium-results/longshot/targets.json 2>/dev/null) targets)" || echo "none"`
...
All longshot artifacts live under `vigolium-results/longshot/` to keep them quarantined from `vigolium-results/findings-draft/` and `vigolium-results/findings/` (which belong to deep/balanced):

```
vigolium-results/longshot/
...
Curated findings stay in `vigolium-results/longshot/findings-draft/longshot-curated-*.md`. They are NOT auto-promoted into `vigolium-results/findings/` — the user can promote manually after reviewing the summary.

### Pre-Flight Check

If `vigolium-results/longshot/targets.json` already exists, use `AskUserQuestion`:

- **Incomplete run**: "A longshot run is already in progress. What would you like to do?"
  - Options: "Resume from last checkpoint" | "Start fresh (clears vigolium-results/longshot/)" | "Cancel"
...
**Start fresh**: `rm -rf vigolium-results/longshot/` then proceed.

Do not proceed past pre-flight without an explicit user choice.

### Pre-Audit Setup

1. `mkdir -p vigolium-results/longshot/findings-draft/`

The orchestrator engine owns `vigolium-results/audit-state.json` — do not write to it from inside this command-def. Per-phase status transitions (`in_progress` → `complete`/`failed`) are recorded automatically when each phase ends.

---

## Phase 1: Enumerate Targets

This is an inline phase. You walk the repo, score every candidate source file by danger heuristics, and write the ranked list to `vigolium-results/longshot/targets.json`.
...
vigolium-audit third_party third-party
```
...
Write `vigolium-results/longshot/targets.json`:
...
Dispatch `vigolium-audit:longshot-collector` (single foreground run, no fan-out). It reads every draft under `vigolium-results/longshot/findings-draft/`, deduplicates by root cause, ranks by severity+confidence, writes curated drafts (`longshot-curated-NNN-<slug>.md`), and writes the final report at `vigolium-results/longshot/longshot-summary.md`.

Verify `vigolium-results/longshot/longshot-summary.md` exists and is non-empty after the aggregator finishes.
...
Longshot Audit Complete
  ...
  Summary report:     vigolium-results/longshot/longshot-summary.md
  Curated drafts:     vigolium-results/longshot/findings-draft/longshot-curated-*.md

  Next steps:
    - Review the summary report; promote curated findings into vigolium-results/findings/ manually.
    - Run /vigolium-audit:confirm against any high-confidence finding to build a live PoC.
```
